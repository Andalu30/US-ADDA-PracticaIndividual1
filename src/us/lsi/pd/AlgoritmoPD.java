...
public class AlgoritmoPD<S,A> extends AbstractAlgoritmo {
	...
	private void actualizaMejorValor(ProblemaPD<S,A> p){
			...
			if (objetivo!=null &&
				...
				if (AbstractAlgoritmo.metricasOK)
					AbstractAlgoritmo.metricas.addActualizacionMejorValor();
				mejorValor = objetivo;
			}
	}
	...
	private Sp<A> pD(ProblemaPD<S,A> p){
		if(AbstractAlgoritmo.metricasOK) AbstractAlgoritmo.metricas.addLLamadaRecursiva();		
		Sp<A> e = null;	
		if (haSidoResueltoYa(p)){
			if(AbstractAlgoritmo.metricasOK) AbstractAlgoritmo.metricas.addUsoDeLaMemoria();
			...
			for(A a: randomize(p,p.getAlternativas())){
				if(conFiltro && isMin() && p.getObjetivoEstimado(a) >= mejorValor) {
					if(AbstractAlgoritmo.metricasOK) AbstractAlgoritmo.metricas.addUnFiltro();
					continue;
				}
